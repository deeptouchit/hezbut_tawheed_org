...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        // ৩টি বিশেষ ক্যাটাগরির সর্বশেষ পোস্ট (হেযবুত তওহীদ আকিদা ও বিশ্বাস, হেযবুত তওহীদ সম্পর্কে অপপ্রচারের জবাব, আমরা উগ্রবাদী গোষ্ঠীর হামলার শিকার)
        $featuredCategories = [
            [
                ...
                'category' => BlogCategory::where('slug', 'ideology-beliefs')->first() ?: BlogCategory::where('slug', 'ideology-and-religion')->first(),
                'post' => Blog::published()
                    ->whereHas('category', function ($q) {
                        $q->where('slug', 'ideology-beliefs');
                    })
                    ->with(['author', 'category'])
                    ->latest('published_at')
                    ->latest('id')
                    ->first() ?: Blog::published()
                    ->whereHas('category', function ($q) {
                        $q->where('slug', 'ideology-and-religion');
                    })
                    ->with(['author', 'category'])
                    ->latest('published_at')
                    ->latest('id')
            ...
            [
                'title' => 'হেযবুত তওহীদ সম্পর্কে অপপ্রচারের জবাব',
                'slug' => 'rebuttal-and-legal',
                'icon' => 'fas fa-scale-balanced',
                'theme_color' => '#1d4ed8',
                'theme_bg' => '#eff6ff',
                'badge_text' => 'অপপ্রচারের জবাব',
                'category' => BlogCategory::where('slug', 'rebuttal-and-legal')->first(),
                'post' => Blog::published()
                    ->whereHas('category', function ($q) {
                        $q->where('slug', 'rebuttal-and-legal');
                    })
                    ->with(['author', 'category'])
                    ->latest('published_at')
                    ->latest('id')
            ...
            [
                ...
                'category' => BlogCategory::where('slug', 'history-of-persecution')->first(),
                'post' => Blog::published()
                    ->whereHas('category', function ($q) {
                        $q->where('slug', 'history-of-persecution');
                    })
                    ->with(['author', 'category'])
                    ->latest('published_at')
                    ->latest('id')
    ...
    public function page($slug)
    {
        ...
        if ($slug === 'publications') {
            // 1. Fetch 16 popular books from DB, sorted by popular_order
