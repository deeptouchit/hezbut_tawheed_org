                        ...
                        <div class="card card-outline card-success shadow-sm mb-4">
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="slug" id="slug" class="form-control @error('slug') is-invalid @enderror" placeholder="যেমন: vision, about" value="{{ old('slug', $page->slug) }}" required>
                                    <small class="text-muted">পেজটির ইউআরএল হবে: <span class="fw-bold" id="slugPreview">{{ url('/') }}/{{ $page->slug }}</span></small>
                                    @error('slug')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea name="content" id="editor" class="form-control @error('content') is-invalid @enderror" rows="15">{{ old('content', $page->content) }}</textarea>
                                    @error('content')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                    ...
                    <div class="col-md-3">
                        ...
                        <div class="form-section shadow-sm bg-white border border-light">
                            ...
                            <div class="form-check form-switch mb-3">
                                <input class="form-check-input" type="checkbox" name="is_active" id="statusSwitch" value="1" {{ $page->is_active ? 'checked' : '' }}>
                                <label class="form-check-label fw-bold" for="statusSwitch" id="statusLabel">{{ $page->is_active ? 'সক্রিয় (Active)' : 'নিষ্ক্রিয় (Inactive)' }}</label>
                            </div>
