...
@push('styles')
    <style>
        ...
        .leader-item-card.level-1 {
            margin-left: 40px;
            border-left: 5px solid #006A4E !important;
            /* Level 1: Deep Green */
        }
...
@section('content')
    <div class="container-fluid">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            <div>
                <h3 class="mb-1 fw-bold text-dark-green"><i class="fas fa-sitemap me-1"></i> সাংগঠনিক হায়ারার্কি ডায়াগ্রাম
                    সাজান</h3>
        ...
        <div class="alert alert-info alert-dismissible fade show" role="alert">
            ...
            <ul class="mb-0 ps-3 small">
                <li>যেকোনো মেম্বারকে ড্র্যাগ হ্যান্ডেল (<i class="fas fa-grip-vertical"></i>) ধরে টেনে উপরে-নিচে নামিয়ে
                    পজিশন বা সিরিয়াল পরিবর্তন করতে পারবেন।</li>
                <li>ডান পাশের অ্যারো বোতাম (<i class="fas fa-arrow-right"></i>) দিয়ে মেম্বারকে ডানে সরিয়ে (Indent) যেকোনো
                    নেতার অধীনস্থ বা সাব-লিডার করতে পারবেন।</li>
                <li>বাম পাশের অ্যারো বোতাম (<i class="fas fa-arrow-left"></i>) দিয়ে মেম্বারকে বামে সরিয়ে (Outdent) ওপরের
                    স্তরে উন্নীত করতে পারবেন।</li>
                <li>হায়ারার্কি সাজানো শেষ হলে ডানদিকের <strong>"হায়ারার্কি সংরক্ষণ করুন"</strong> বাটনে ক্লিক করে সেভ করুন।
                </li>
            </ul>
                ...
                <div class="row align-items-center justify-content-between">
                    <div class="col-md-6 col-lg-5">
                        <div class="input-group">
                            ...
                            <input type="text" id="leader-search-input" class="form-control bg-light border-start-0"
                                placeholder="নেতৃত্বের নাম বা পদবি দিয়ে খুঁজুন...">
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-4 text-md-end mt-2 mt-md-0">
                        <span class="badge bg-secondary py-2 px-3 fs-7" id="showing-count-badge">মোট সদস্য:
                            {{ count($leaders) }} জন</span>
                    </div>
                </div>
            </div>
            <div class="card-body bg-light">
                <div id="leadersList" class="leaders-list-container">
                    @forelse($leaders as $leader)
                        <div class="leader-item-card level-{{ $leader->level }}" data-id="{{ $leader->id }}"
                            data-parent-id="{{ $leader->parent_id }}" data-level="{{ $leader->level }}">
                            ...
                            <div class="d-flex align-items-center">
                                ...
                                <span class="nesting-indicator"
                                    style="display: {{ $leader->parent_id ? 'inline-block' : 'none' }};">
                                    <i class="fas fa-level-up-alt fa-rotate-90"></i>
                                </span>

                                <!-- Profile avatar -->
                                <img src="{{ $leader->image_url }}" alt="{{ $leader->name }}" class="rounded me-3 border"
                                ...
                                <div>
                                    ...
                                    <span class="text-muted small">
                                        {{ $leader->designation }}
                                        <span class="badge bg-light text-muted border ms-1 level-badge"
                                            style="font-size: 9px;">স্তর {{ $leader->level + 1 }}</span>
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="d-flex gap-1">
                                <button class="indent-action-btn outdent-item-btn" title="বাম দিকে আনুন"
                                    style="display: {{ $leader->parent_id ? 'inline-flex' : 'none' }};">
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            // Helper to get all descendants of a card
            function getDescendants(item) {
                let level = parseInt(item.attr('data-level')) || 0;
                let descendants = [];
                let next = item.next('.leader-item-card');
                while (next.length && (parseInt(next.attr('data-level')) || 0) > level) {
                    descendants.push(next);
                    next = next.next('.leader-item-card');
                }
                return descendants;
            }
            ...
            if ($.fn.sortable) {
                $('#leadersList').sortable({
                    ...
                    helper: function(event, item) {
                        // Create a temporary container to hold the parent and its descendants visually
                        let level = parseInt(item.attr('data-level')) || 0;
                        let descendants = getDescendants(item);
                        let helperContainer = $('<div class="sortable-drag-helper" style="width: ' +
                            item.outerWidth() + 'px; z-index: 9999;"></div>');

                        // Clone the dragged item and append
                        let itemClone = item.clone();
                        helperContainer.append(itemClone);

                        // Clone and append descendants with their relative indentation
                        descendants.forEach(function(desc) {
                            let descClone = desc.clone();
                            let childLevel = parseInt(descClone.attr('data-level')) || 0;
                            let relativeLevel = childLevel - level;
                            descClone.css({
                                'margin-left': (relativeLevel * 40) + 'px',
                                'margin-top': '8px',
                                'opacity': '0.9'
                            });
                            helperContainer.append(descClone);
                        });
                    ...
                    start: function(event, ui) {
                        let item = ui.item;
                        let descendants = getDescendants(item);
                        item.data('descendants', descendants);

                        // Hide original descendants in the list so they don't show twice
                        descendants.forEach(function(desc) {
                            desc.hide();
                        });

                        // Adjust placeholder height to include space for descendants
                        let totalHeight = item.outerHeight();
                        descendants.forEach(function(desc) {
                            totalHeight += desc.outerHeight() + 8; // include margin
                        });
                        ui.placeholder.height(totalHeight);

                        // Refresh sortable positions
                        $(this).sortable('refreshPositions');
                    },
                    stop: function(event, ui) {
                        let item = ui.item;
                        let descendants = item.data('descendants') || [];

                        // Place descendants right after the dropped item in the list and show them
                        let prev = item;
                        descendants.forEach(function(desc) {
                            desc.insertAfter(prev);
                            desc.show();
                            prev = desc;
                        });

                        // Recalculate level of the dragged item
                        let oldLevel = parseInt(item.attr('data-level')) || 0;
                        adjustDraggingLevel(item, ui.helper);
                        let newLevel = parseInt(item.attr('data-level')) || 0;
                        let levelDiff = newLevel - oldLevel;

                        // Shift all descendants by the level difference
                        if (levelDiff !== 0) {
                            descendants.forEach(function(desc) {
                                let childOldLevel = parseInt(desc.attr('data-level')) || 0;
                                let childNewLevel = Math.max(0, Math.min(6, childOldLevel +
                                    levelDiff));
                                setItemLevel(desc, childNewLevel);
                            });
                        }
            ...
            function adjustDraggingLevel(item, helper) {
                let containerLeft = $('#leadersList').offset().left;
                let activeElement = (helper && helper.length) ? helper : item;
                let cardLeft = activeElement.offset().left;
                let relativeLeft = cardLeft - containerLeft;

                // Estimate level based on indent spacing (40px per level)
                let estimatedLevel = Math.max(0, Math.min(6, Math.round(relativeLeft / 40)));
            ...
            function setItemLevel(item, level) {
                ...
                item.removeClass('level-0 level-1 level-2 level-3 level-4 level-5 level-6');
            ...
            function updateLeadersTreeStructure() {
                ...
                $('#leadersList .leader-item-card').each(function() {
                    ...
                    let precedingLevel = preceding.length ? (parseInt(preceding.attr('data-level')) || 0) :
                        -1;
                    ...
                    if (precedingLevel >= level && level < 6) {
                        indentBtn.show();
                    } else {
                        indentBtn.hide();
                    }
            ...
            $(document).on('click', '.indent-item-btn', function(e) {
                ...
                if (level <= precedingLevel && level < 6) {
                    let descendants = getDescendants(card);

                    setItemLevel(card, level + 1);

                    // Shift all descendants too
                    descendants.forEach(function(desc) {
                        let descLevel = parseInt(desc.attr('data-level')) || 0;
                        setItemLevel(desc, Math.min(6, descLevel + 1));
                    });
            ...
            $(document).on('click', '.outdent-item-btn', function(e) {
                ...
                if (level > 0) {
                    ...
                    descendants.forEach(function(desc) {
                        let descLevel = parseInt(desc.attr('data-level')) || 0;
                        setItemLevel(desc, Math.max(0, descLevel - 1));
                    });
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            toastr.success(response.message ||
                                'সাংগঠনিক হায়ারার্কি সফলভাবে সেভ করা হয়েছে!');
                        } else {
