        ...
        <tbody>
            @forelse ($users as $user)
                @php
                    ...
                    $isTargetSuperAdmin = $role === 'super_admin';
                    $isAuthSuperAdmin = auth()->user()->hasRole('super_admin');
                    $isProtectedFromCurrentAdmin = $isTargetSuperAdmin && !$isAuthSuperAdmin;
                @endphp
                <tr class="text-center">
                    <td>
                        @if ($isProtectedFromCurrentAdmin)
                            <input type="checkbox" class="user-checkbox" value="{{ $user->id }}" disabled
                                title="সুপার এডমিন সুরক্ষিত">
                        @else
                    ...
                    <td>
                        <span class="badge bg-{{ $badgeColor }}"
                            style="padding: 4px 8px; border-radius: 4px;">{{ ucfirst($role) }}</span>
                    </td>
                    <td>
                        @php
                            $status = $user->status ?? 'active';
                            $statusBadge =
                                $status == 'active' ? 'success' : ($status == 'inactive' ? 'danger' : 'warning');
                        @endphp
                        @if ($isProtectedFromCurrentAdmin)
                            <span class="badge bg-{{ $statusBadge }}"
                                style="padding: 5px 8px; border-radius: 4px; opacity: 0.85;"
                                title="সুপার এডমিনের স্ট্যাটাস পরিবর্তনযোগ্য নয়">
                                <i class="fas fa-lock me-1" style="font-size: 0.75rem;"></i> {{ ucfirst($status) }}
                            </span>
                        @else
                            <button type="button" class="btn btn-sm btn-{{ $statusBadge }} toggle-status"
                        ...
                        <div class="btn-group btn-group-sm">
                            ...
                            @if ($isProtectedFromCurrentAdmin)
                                <button type="button" class="btn btn-secondary disabled"
                                    title="লক করা - সুপার এডমিন অ্যাকাউন্ট" disabled style="opacity: 0.65;">
                            ...
                            @else
                                ...
                                <button type="button" class="btn btn-danger delete-user" data-id="{{ $user->id }}"
                                    data-name="{{ $user->name }}" title="ডিলিট">
