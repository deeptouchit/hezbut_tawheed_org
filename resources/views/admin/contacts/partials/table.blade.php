    ...
    <table class="table table-bordered table-striped table-hover table-sm text-nowrap align-middle" style="width: 100%; font-size: 13px;">
        <thead class="">
            <tr class="text-center">
                ...
                <th>নাম</th>
                <th>ইমেইল</th>
                <th style="max-width: 180px;">বিষয়</th>
                <!-- <th>বার্তা</th> -->
                <th>স্ট্যাটাস</th>
                <th>তারিখ</th>
                <th>অ্যাকশন</th>
        ...
        <tbody>
            ...
            <tr class="text-center table-row-hover {{ $message->status == 'unread' ? 'unread-row' : ($message->status == 'replied' ? 'replied-row' : '') }}"
                data-id="{{ $message->id }}">
                <td>
                ...
                <td class="text-start" style="max-width: 180px;" title="{{ $message->subject ?? '' }}">
                    @if($message->subject)
            ...
            <tr>
                <td colspan="9" class="text-center py-5">
                    ...
                    <p class="text-muted mb-0">কোন যোগাযোগ বার্তা পাওয়া যায়নি</p>
...
<script>
    $(document).ready(function() {
        ...
        $('#reset-filter-empty').off('click').on('click', function() {
            $('#search-input').val('');
            $('#status-filter').val('');
        ...
        // Mark as Read (Single)
        // ============================================
        $('.mark-read-single').off('click').on('click', function() {
            var id = $(this).data('id');
            var btn = $(this);
