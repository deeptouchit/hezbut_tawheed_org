...
@section('title', $category->name . ' - হেজবুত তওহীদ')
...
@section('content')
    ...
    @php
        $isLiveUpdateCategory = $category->slug === 'live-update';
    @endphp
    ...
    <div class="py-5" style="background-color: #f8fafc; min-height: 70vh;">
        <div class="container">
            <div class="row">
                ...
                <!-- Blog Listing Column -->
                <div class="{{ $isLiveUpdateCategory ? 'col-12' : 'col-lg-8' }}">
                    @if (!$isLiveUpdateCategory)
                        <!-- Category active filter notice -->
                        <div class="bg-white p-3 rounded-3 shadow-sm border border-light-grey mb-4 d-flex justify-content-between align-items-center"
                            style="font-family: 'Baloo Da 2', sans-serif; font-size: 0.92rem;">
                            <span class="text-secondary">
                                বর্তমানে প্রদর্শিত হচ্ছে ক্যাটাগরি: <strong
                                    class="text-success">"{{ $category->name }}"</strong> এর নিবন্ধসমূহ
                            </span>
                            <a href="{{ route('blog') }}" class="btn btn-sm btn-outline-success rounded-3 px-3 py-1 fw-bold"
                                style="font-size: 11px;">সকল নিবন্ধ দেখুন</a>
                        </div>
                    @endif
                    ...
                    <div class="row g-4" id="blog-posts-container">
                        ...
                        @empty
                            <div class="col-12 text-center py-5 bg-white rounded-3 shadow-sm border border-light">
                                ...
                                <h4 class="text-dark fw-bold" style="font-family: 'Baloo Da 2', sans-serif;">দুঃখিত, এই
                                    ক্যাটাগরিতে কোনো নিবন্ধ পাওয়া যায়নি!</h4>
                                <p class="text-secondary small">অনুগ্রহ করে পরবর্তীতে আবার চেষ্টা করুন বা অন্যান্য
                                    ক্যাটাগরির নিবন্ধসমূহ ব্রাউজ করুন।</p>
                                ...
                                <div class="d-flex flex-column gap-1.5" style="font-family: 'Baloo Da 2', sans-serif;">
                                    @foreach ($categories as $catItem)
                                        <a href="{{ route('blog.category', $catItem->slug) }}"
                                            ...
                                            <span
                                                class="badge category-count-badge">{{ $catItem->blogs_count ?? $catItem->blogs()->published()->count() }}</span>
                                        </a>
                                    @endforeach
                                </div>
                            </div>
